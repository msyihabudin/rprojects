...
#month
summary(data$month) #untuk cek data yang paling banyak
data$month[data$month == ""] <- names(sort(-table(data$month))[1])
#day
summary(data$day)#untuk cek data yang paling banyak
data$day[data$day == ""] <- names(sort(-table(data$day))[1])
#FFMC
